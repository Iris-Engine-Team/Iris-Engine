...
#define LIGHT_DESCRIPTOR 2
#define SHADOW_DESCRIPTOR 3
#define IBL_DESCRIPTOR 4

layout(location = 0) in vec3 cameraPosition;

layout(location = 0) out vec4 fragColor;

layout(input_attachment_index = 0, set = 1, binding = 0) uniform subpassInput positionTexture;
layout(input_attachment_index = 1, set = 1, binding = 1) uniform subpassInput normalTexture;
layout(input_attachment_index = 1, set = 1, binding = 2) uniform subpassInput albedoTexture;
layout(input_attachment_index = 3, set = 1, binding = 3) uniform subpassInput materialTexture;
...
void main()
{
    vec3 vertexWorldPosition = vec3(subpassLoad(positionTexture));
    vec4 albedo = subpassLoad(albedoTexture);
    vec4 normal = subpassLoad(normalTexture);
    vec4 material = subpassLoad(materialTexture);

    vec3 normalVec = vec3(normal);
    ...
    for(int i = 0; i < LightDataUBO.dirLightCount; ++i)
    {
        ...
        brdf = BRDF(V, normalVec, -normalize(vec3(LightDataUBO.dirLight[i].direction)), roughness, metallic, vec3(albedo), specular, (1.0 - shadow));
    ...
    for(int i = 0; i < LightDataUBO.spotLightCount; ++i)
    {
        ...
        brdf = BRDF(V, normalVec, normalize(vec3(LightDataUBO.spotLight[i].position) - vec3(vertexWorldPosition)), roughness, metallic, vec3(albedo), specular, (1.0 - shadow));
    ...
    for(int i = 0; i < LightDataUBO.pointLightCount; ++i)
    {
        ...
        brdf = BRDF(V, normalVec, normalize(vec3(LightDataUBO.pointLight[i].position) - vec3(vertexWorldPosition)), roughness, metallic, vec3(albedo), specular, (1.0 - shadow));
	...
	_F0 = mix(_F0, vec3(albedo), metallic);
	...
	vec3 diffuse = irraAmbiant * vec3(albedo);
